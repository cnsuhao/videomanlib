...
{
	Error error = cameraList[0]->cam.WriteRegister( reg, value ); 	
...
{
	CameraInfo camInfo;
	Error error = cameraList[0]->cam.GetCameraInfo(&camInfo);
    if (error == PGRERROR_OK)    
		PrintInfo(&camInfo);
}
...
void PointGreyController::convertRawToColor( const unsigned char* imgSrc, unsigned char *imgDst, int width, int height )
{
	...
	CameraInfo camInfo;
	cameraList[0]->cam.GetCameraInfo(&camInfo);	
	rawImage.SetDimensions( height, width, width, PIXEL_FORMAT_RAW8, camInfo.bayerTileFormat );
