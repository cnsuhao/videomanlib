...
PGRGigECamera::PGRGigECamera(void)
{
	//recording = false;
	//started = false;
...
FlyCapture2::FrameRate PGRGigECamera::buildFrameRate( double fps )
{
	if ( fps == 1.875 )
...
FlyCapture2::VideoMode PGRGigECamera::buildVideoMode( VMInputFormat format )
{	
...
void PGRGigECamera::getFirstPixelFormatSupported(const FlyCapture2::Format7Info &fmt7Info,FlyCapture2::PixelFormat &fmt7PixFmt)
{
...
FlyCapture2::PixelFormat PGRGigECamera::buildPixelFormat( VMPixelFormat pixelFormat )
{
	switch( pixelFormat )
	{
		case VM_RGB24:
...
{
	PGRGigECamera *pgrCamerap = (PGRGigECamera*)pCallbackData;
...
void PGRGigECamera::setFrameCallback( getFrameCallback theCallback, void *data )
{
...
bool PGRGigECamera::resetCamera( )
{
	Error error = m_camera.WriteRegister( INITIALIZE,0x80000000 );
	return ( error != PGRERROR_OK );
}

bool PGRGigECamera::initCamera( unsigned long aSerialNumber, VMInputFormat *aFormat )
{
    ...
    CameraInfo camInfo;
    Error error = m_camera.GetCameraInfo(&camInfo);
    if (error != PGRERROR_OK)
    {
        PrintError( error );
        return false;
	...
	//It is a GigE camera?
	bool GigECamera = ( camInfo.interfaceType == INTERFACE_GIGE );	
	...
	if ( aFormat == NULL || aFormat->showDlg )
	{
		...
		bool availableFormat = resolveFormat( imageSettings, format );	
		if ( !availableFormat )
			return false;
	}
	else if ( aFormat != NULL )
	{
		...
		m_camera.GetGigEImageSettingsInfo( &imageSettingsInfo );
		
		GigEImageSettings imageSettings;
		imageSettings.offsetX = 0;
		imageSettings.offsetY = 0;
		imageSettings.height = aFormat->height;
		imageSettings.width = aFormat->width;
		imageSettings.pixelFormat = buildPixelFormat( aFormat->getPixelFormatOut() );
		error = m_camera.SetGigEImageSettings( &imageSettings );
		if (error != PGRERROR_OK)
		{
			PrintError( error );
			return false;
...
void PGRGigECamera::releaseFrame()
{
	rawImage.ReleaseBuffer();
	lastImage = NULL;
	pixelBuffer = NULL;	
...
inline char *PGRGigECamera::getFrame( bool wait )
{
	//return pixelBuffer;
...
void PGRGigECamera::showPropertyPage()
{
	//DWORD dwThreadID;
...
VideoManInputController *PGRGigECamera::getController()
{
	return (VideoManInputController*)( controller );
}

bool PGRGigECamera::linkController( VideoManInputController *acontroller )
{
...
int PGRGigECamera::getNumberOfLostFrames()
{
	//return lostFrames;
	return 0;
}

VMPixelFormat PGRGigECamera::resolvePixelFormat( FlyCapture2::PixelFormat pFormat )
{
	switch( pFormat )
	{
...
bool PGRGigECamera::resolveFormat( VideoMode videoMode, FrameRate frameRate, VMInputFormat &rFormat )
{
	switch ( videoMode )
	{
		...
		case VIDEOMODE_FORMAT7:
		{
			...
			Error error = m_camera.GetGigEImagingMode( &mode );
			...
			bool supported;			
			error = m_camera.GetGigEImagingMode( &mode );
			if (error != PGRERROR_OK)
			{
				PrintError( error );
				return false;
...
void PGRGigECamera::getAvailableDevices(  VMInputIdentification **deviceList, int &numDevices )
{	
	numDevices = 0;
	*deviceList = NULL;
...
bool PGRGigECamera::setGainControl( bool autoGain )
{
	Property prop;
	prop.type = GAIN;
...
bool PGRGigECamera::getGainControl()
{
	Property prop;
	prop.type = GAIN;
	m_camera.GetProperty( &prop );
...
bool PGRGigECamera::setExposureControl( bool autoExp )
{
	Property prop;
...
bool PGRGigECamera::setSharpnessControlValue(float value)
{
	Property prop;
	prop.type = SHARPNESS;
	m_camera.GetProperty( &prop );
...
bool PGRGigECamera::setSharpnessControl( bool autoSharp )
{
	Property prop;
	prop.type = SHARPNESS;
	m_camera.GetProperty( &prop );
...
bool PGRGigECamera::setShutterControl( bool autoShutter )
{
	Property prop;
	prop.type = SHUTTER;
	m_camera.GetProperty( &prop );
...
bool PGRGigECamera::setShutterTime( float shutterTime )
{
	Property prop;
	prop.type = SHUTTER;
	m_camera.GetProperty( &prop );
...
bool PGRGigECamera::moveImageROI( int x, int y )
{
...
unsigned int PGRGigECamera::getRegisterValue(unsigned long _register )
{
	unsigned int value;
...
bool PGRGigECamera::getROIUnits( int &Hmax, int &Vmax, int &Hunit, int &Vunit, int &HPosUnit, int &VPosUnit )
{
	if ( m_videoMode == VIDEOMODE_FORMAT7 )
	{
...
bool PGRGigECamera::resetImageROI()
{
	return setImageROI( 0, 0, m_fmt7Info.maxWidth, m_fmt7Info.maxHeight );
}

int PGRGigECamera::buildColorCoding( VMPixelFormat pixelFormat )
{
	switch( pixelFormat )
	{
	case VM_GREY8:
...
bool PGRGigECamera::setTrigger( bool triggerOn, int source, int mode )
{
	TriggerMode triger;
...
bool PGRGigECamera::fireSoftwareTrigger( bool broadcast )
{
	return ( m_camera.FireSoftwareTrigger( broadcast ) == PGRERROR_OK );
}

bool PGRGigECamera::setStrobeOutput( bool onOff, float delay, float duration,	unsigned int polarity, unsigned int source )
{
...
bool PGRGigECamera::setFrameRate( double frameRate )
{
	Property prop;
	prop.type = FRAME_RATE;
	m_camera.GetProperty( &prop );
