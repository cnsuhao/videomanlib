...
const char *identifiers[1] = {"GENICAM_CAMERA"};
const int numIdentifiers = 1;
const char *controllersIdentifiers[1] = {"GENICAM_CAMERA_CONTROLLER"};
const int numControllersIdentifiers = 1;

#ifdef WIN32
#define VMGenicam_API  __declspec(dllexport)
#elif defined linux
#define VMGenicam_API
#endif
...
//extern "C" VMGenicam_API VideoInput *initVideoInput( const VMInputIdentification &device, VMInputFormat *format );
VideoInput *initVideoInput( const VMInputIdentification &device, VMInputFormat *format )
{
	return NULL;	
}

//extern "C" VMGenicam_API void getAvailableDevices( VMInputIdentification **deviceList, int &numDevices );
...
//extern "C" VMGenicam_API const char **getIdentifiers( int &numIdentifiers );
...
//extern "C" VMGenicam_API char* checkVersion( );
char* checkVersion( )
{
	#ifdef _DEBUG
		return "DEBUG" ;
...
//extern "C" VMGenicam_API void deleteInput( VideoInput **input );
...
//extern "C" VMGenicam_API const char **getControllerIdentifiers( int &numIdentifiers );
...
//extern "C" VMGenicam_API VideoManInputController *createController( const char *identifier  );
...
//extern "C" VMGenicam_API void deleteController( VideoManInputController **controller );
